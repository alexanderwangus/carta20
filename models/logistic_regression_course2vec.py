...
from course_embeddings.course2vec import get_course2vec_model_path, train_model, featurize_student, create_training_set
...
def log_reg_course2vec(training_set=None, vec_size=150, win_size=10, min_count=2, epochs=10, num_classes_val=-1):
    ...
    # prep datasets
    X_train, X_val, X_test, y_train, y_val, y_test = util.prep_dataset(vectorize=False, num_classes_val=num_classes_val)
...
def main():
    ...
    log_reg_course2vec(training_set=training_set, vec_size=200, win_size=10, min_count=2, epochs=10)
